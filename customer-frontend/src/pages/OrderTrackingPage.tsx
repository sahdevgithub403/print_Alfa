...
import { CheckCircle2, Printer, ArrowLeft, RefreshCw, FileText } from 'lucide-react';
...
export const OrderTrackingPage: React.FC = () => {
  ...
  const getStepStatus = (stepName: 'RECEIVED' | 'PRINTING' | 'READY') => {
    ...
    <div className="min-h-screen bg-[#F5F5F3] text-[#111111] flex flex-col">
      ...
      <header className="bg-white border-b border-[#E2E2E2] py-5 px-6 sticky top-0 z-30 shadow-2xs">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          ...
          >
            <RefreshCw className={`w-5 h-5 ${isRefreshing ? 'animate-spin text-[#111111]' : ''}`} />
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-xl w-full mx-auto px-5 sm:px-6 py-10 space-y-8">
        ...
        <div className="bg-white rounded-2xl p-8 border border-[#E2E2E2] space-y-8 shadow-2xs">
          <div className="space-y-6 relative before:absolute before:left-5 before:top-4 before:bottom-4 before:w-0.5 before:bg-[#E2E2E2]">
            ...
            <div className="flex items-start gap-5 relative z-10">
              ...
              <div className="pt-1">
                <h3 className="text-lg font-bold text-[#111111]">Order Received</h3>
                <p className="text-sm text-[#6B6B6B] mt-0.5">Sent directly to shop queue</p>
              </div>
            </div>

            {/* Step 2: Printing */}
            <div className="flex items-start gap-5 relative z-10">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 border-4 border-white shadow-xs ${
              ...
              <div className="pt-1">
                <h3 className="text-lg font-bold text-[#111111]">Printing Files</h3>
                <p className="text-sm text-[#6B6B6B] mt-0.5">
                  {order?.printStatus === 'PRINTING' ? 'Operator is currently printing your files' : 'Waiting in printer queue'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-5 relative z-10">
              ...
              <div className="pt-1">
                <h3 className="text-lg font-bold text-[#111111]">Ready for Pickup</h3>
                <p className="text-sm text-[#6B6B6B] mt-0.5">Collect prints at shop counter</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4 divide-y divide-[#E2E2E2]">
              {itemsList.length > 0 ? (
                  ...
                  <div key={item.id} className={`flex items-start justify-between gap-4 ${idx > 0 ? 'pt-4' : ''}`}>
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-base font-bold text-[#111111] truncate">{item.document?.originalFileName || `File ${idx + 1}`}</p>
                        <p className="text-xs text-[#6B6B6B] mt-0.5 font-medium">
                          {item.colorMode === 'BW' ? 'B&W' : 'Color'} · {item.paperSize} · {item.copies} {item.copies === 1 ? 'copy' : 'copies'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="shrink-0 text-right">
                      <span className={`inline-block px-2.5 py-1 rounded-md text-xs font-extrabold uppercase ${
                        item.printStatus === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' :
                        item.printStatus === 'PRINTING' ? 'bg-amber-100 text-amber-800' :
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#111111] text-base">₹{order.totalPrice.toFixed(2)}</span>
                </div>
              )}
            </div>

            <div className="flex items-center justify-between pt-4 border-t border-[#E2E2E2]">
              <span className="text-[#6B6B6B] text-sm font-medium">Total Amount</span>
              <span className="text-xl font-extrabold text-[#111111]">₹{order.totalPrice.toFixed(2)}</span>
            </div>
          </div>
        )}
      </main>
