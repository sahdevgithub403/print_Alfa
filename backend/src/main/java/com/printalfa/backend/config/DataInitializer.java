...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Shop shop = shopRepository.findBySlug("quickprint")
                .orElseGet(() -> {
                    Shop s = new Shop("QuickPrint Jamshedpur", "quickprint", "Bistupur Main Road, Jamshedpur, Jharkhand", "+91 98765 43210", null);
                    s.setApiKey("pa_live_quickprint_sec_12345");
                    return shopRepository.save(s);
                });
        if (shop.getApiKey() == null) {
            shop.setApiKey("pa_live_quickprint_sec_12345");
        ...
        Shop shopB = shopRepository.findBySlug("campusprint")
                .orElseGet(() -> {
                    Shop s = new Shop("CampusPrint NIT", "campusprint", "Campus Gate No 2, NIT Adityapur", "+91 98765 99999", null);
                    s.setApiKey("pa_live_campusprint_sec_67890");
                    return shopRepository.save(s);
                });
        if (shopB.getApiKey() == null) {
            shopB.setApiKey("pa_live_campusprint_sec_67890");
        ...
        // 4. Seed Documents and Sample Orders if empty
        if (printOrderRepository.count() == 0) {
            ...
            // Seed Initial Sample Order 1 (PR-1024)
            ...
            order1.setShop(shop);
            order1.setCustomerName("Aarav Sharma");
            order1.setCustomerPhone("9876543210");
            ...
            item1.setCopies(2);
            item1.setCalculatedPages(8);
            item1.setUnitPrice(new BigDecimal("2.00"));
            item1.setItemPrice(new BigDecimal("32.00"));
            item1.setPrintStatus(PrintStatus.PENDING);

            order1.setItems(new ArrayList<>(List.of(item1)));
            printOrderRepository.save(order1);
            printJobRepository.save(new PrintJob(order1, JobStatus.QUEUED));

            // Seed Initial Sample Order 2 (PR-1025)
            ...
            order2.setShop(shop);
            order2.setCustomerName("Priya Patel");
            order2.setCustomerPhone("9876543211");
            ...
            order2.setRazorpayOrderId("order_demo_1025");
            order2.setRazorpayPaymentId("pay_demo_1025");
            ...
            item2.setDocument(doc2);
            item2.setPrintType(PrintType.PRINT);
            item2.setColorMode(ColorMode.COLOR);
            item2.setPaperSize(PaperSize.A4);
            item2.setPrintSide(PrintSide.DOUBLE);
            item2.setPageRange("1-10");
            item2.setCopies(1);
            item2.setCalculatedPages(10);
            item2.setUnitPrice(new BigDecimal("18.00"));
            item2.setItemPrice(new BigDecimal("180.00"));
            item2.setPrintStatus(PrintStatus.PRINTING);

            order2.setItems(new ArrayList<>(List.of(item2)));
            printOrderRepository.save(order2);
            printJobRepository.save(new PrintJob(order2, JobStatus.PROCESSING));

            // Seed Initial Sample Order 3 (PR-1026)
            ...
            order3.setShop(shop);
            order3.setCustomerName("Rohan Gupta");
            order3.setCustomerPhone("9876543212");
            ...
            item3.setItemPrice(new BigDecimal("6.00"));
            item3.setPrintStatus(PrintStatus.COMPLETED);

            order3.setItems(new ArrayList<>(List.of(item3)));
            printOrderRepository.save(order3);
            printJobRepository.save(new PrintJob(order3, JobStatus.COMPLETED));
        }
