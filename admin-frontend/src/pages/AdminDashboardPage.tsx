...
import { getAdminOrders, updateOrderPrintStatus, updateOrderPaymentStatus } from '../api';
...
export const AdminDashboardPage: React.FC<AdminDashboardPageProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    stompClient.onConnect = (frame) => {
      ...
      if (shopId) {
        stompClient.subscribe(`/topic/admin/shop/${shopId}/orders`, (message) => {
          if (message.body) {
            ...
            if (window.electronAPI && event.type === 'NEW_PRINT_REQUEST') {
              ...
              const customerName = event.order?.customerName || 'Walk-in Customer';
  ...
  const fetchOrders = async (isBackground = false) => {
    ...
    try {
      ...
      if (!isBackground) {
        setError('Unable to load shop orders. Please check backend connection.');
      }
    } finally {
      setLoading(false);
  ...
  const printingCount = orders.filter((o) => o.printStatus === 'PRINTING').length;
  const completedCount = orders.filter((o) => o.printStatus === 'COMPLETED').length;

  // Filtered orders logic
  const filteredOrders = orders.filter((order) => {
    if (activeTab === 'HISTORY') {
      if (order.printStatus !== 'COMPLETED' && order.printStatus !== 'CANCELLED') return false;
    ...
    if (searchQuery.trim()) {
      ...
      return filename.includes(q) || orderNum.includes(q) || customer.includes(q) || phone.includes(q);
    }
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    ...
                    {pendingCount > 0 && (
                      <span className={`px-2 py-0.5 rounded-full text-xs font-extrabold ${
                        statusFilter === 'PENDING' ? 'bg-white text-[#111111]' : 'bg-neutral-200 text-neutral-800'
                      }`}>
                        {pendingCount}
                      </span>
                    )}
                  ...
                  >
                    ...
                    {printingCount > 0 && (
                      <span className={`px-2 py-0.5 rounded-full text-xs font-extrabold ${
                        statusFilter === 'PRINTING' ? 'bg-white text-[#111111]' : 'bg-blue-100 text-blue-900'
                      }`}>
                        {printingCount}
                      </span>
                    )}
                ...
                <div className="text-sm text-[#6B6B6B] font-semibold">
                  Showing <span className="font-extrabold text-[#111111]">{filteredOrders.length}</span> print jobs
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-[#E2E2E2] p-16 text-center space-y-4 my-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-[#111111]">No print orders found</h3>
