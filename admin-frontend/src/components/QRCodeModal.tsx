...
export const QRCodeModal: React.FC<QRCodeModalProps> = ({ shopName, shopSlug, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-neutral-900/50 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-[#E2E2E2] p-8 w-full max-w-md space-y-6 shadow-2xl relative animate-in fade-in zoom-in duration-150">
        ...
        <div id="printable-qr-poster" className="border-2 border-[#111111] rounded-2xl p-6 text-center space-y-4 bg-white">
          <div className="space-y-1">
            <div className="w-10 h-10 rounded-xl bg-[#111111] text-white flex items-center justify-center mx-auto mb-2">
              <Printer className="w-6 h-6" />
            </div>
            <h2 className="text-lg font-extrabold text-[#111111] uppercase tracking-tight">{shopName}</h2>
          ...
          <div className="space-y-1">
            <p className="text-sm font-extrabold text-[#111111]">SCAN WITH YOUR PHONE</p>
            <p className="text-xs text-[#6B6B6B]">Upload PDF / Docs · Pay Online or Counter</p>
          </div>
        </div>
