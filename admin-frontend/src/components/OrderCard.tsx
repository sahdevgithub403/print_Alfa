...
export const OrderCard: React.FC<OrderCardProps> = ({
  ...
  const totalCalculatedPages = items.length > 0
    ? items.reduce((sum, item) => sum + (item.calculatedPages || item.document?.pageCount || 1), 0)
    : (order.document?.pageCount || order.calculatedPages || 1);

  const handleDownloadFirst = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!docId) return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-start gap-4 min-w-0 flex-1">
          <div className="w-12 h-12 rounded-xl bg-white border border-[#E2E2E2] text-[#111111] flex items-center justify-center shrink-0 mt-0.5 shadow-2xs">
            {itemCount > 1 ? <Files className="w-6 h-6 text-neutral-800" /> : <FileText className="w-6 h-6 text-neutral-800" />}
          </div>

          <div className="min-w-0 space-y-1">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-bold text-[#111111] truncate" title={primaryDocName}>
                {primaryDocName}
                {itemCount > 1 && (
                  <span className="ml-2 text-xs font-extrabold bg-neutral-200 text-[#111111] px-2 py-0.5 rounded-full inline-block">
                    +{itemCount - 1} more {itemCount - 1 === 1 ? 'file' : 'files'}
                  </span>
                )}
              </h3>
            ...
            <p className="text-base text-[#6B6B6B] font-medium">
              <strong className="text-[#111111] font-semibold">{itemCount || 1} {itemCount === 1 ? 'file' : 'files'}</strong> · <strong className="text-[#111111] font-semibold">{totalCalculatedPages} {totalCalculatedPages === 1 ? 'total page' : 'total pages'}</strong>
              {order.customerName && <span className="ml-2 text-neutral-500">({order.customerName})</span>}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 shrink-0 flex-wrap">
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                onUpdatePaymentStatus(order.id, order.paymentStatus === 'PAID' ? 'PENDING' : 'PAID');
              }}
              className={order.paymentStatus === 'PAID' ? 'chip-payment-paid' : 'chip-payment-pending'}
